...
class IssueGenerator:
    ...
    def check_and_generate(self) -> bool:
        ...
        # Count open issues (excluding pull requests) with retry logic
    ...
    @retry_github_api
    def _get_open_issues_with_retry(self) -> List:
        """Get open issues with retry logic"""
        open_issues = list(self.repo.get_issues(state="open"))
        return [i for i in open_issues if not i.pull_request]
    ...
    @retry_github_api
    def _get_recent_commits_with_retry(self) -> List:
    ...
    def _generate_issues(self, needed: int, open_issues: List) -> None:
        ...
        # Get recent commits with retry logic
        recent_commits = self._get_recent_commits_with_retry()
        commit_messages = "\n".join(
    ...
    def _call_claude(self, prompt: str) -> Optional[str]:
        ...
        try:
            if USE_CLAUDE_CLI:
                ...
                return self._call_anthropic_api_with_retry(prompt)
    ...
    def _parse_and_create_issues(self, response_text: str, needed: int) -> None:
        ...
        try:
            ...
            for issue_data in issues_to_create:
                ...
                new_issue = self._create_issue_with_retry(title, full_body, labels)
